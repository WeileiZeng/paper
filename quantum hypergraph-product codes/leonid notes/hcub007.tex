...
\begin{document}
\title{Higher-dimensional hypergraph-product codes} \author{LPP}
\begin{abstract}
...
error correction\cite{Campbell-2018}, and (c) analysis and
generalization of transformations between QECCs, like the quantum code
enlargement trick by Hastings [ref?].

Origin: (a) Higher-dimensional toric codes and (b) Hypergraph-product
codes\cite{Tillich-Zemor-2009}.  Present construction generalizes
both. 

\section{Construction overview}

I  want to generalize the QHP construction\cite{Tillich-Zemor-2009} to
...
convenient to use the following recursive definition:

\begin{definition}[Dimensional extension of a chain complex]
\label{def:dim-ext} Given an
  $(m-1)$-chain complex $\mathcal{A}\equiv\mathcal{K}(A_1,\ldots,A_{m-1})$:
  $\{0\}\leftarrow C_0\stackrel{A_1}\leftarrow C_1\stackrel{A_2}\leftarrow C_2\ldots
  \stackrel{A_{m-1}}\leftarrow C_{m-1}\stackrel{0_{n_{m-1}\times 0}}\leftarrow \{0\}$ with boundary operators
  $A_j$, $j=1,\ldots,m-1$, and an $r\times c$ binary matrix $P$, the extended
...
The parameters of the CSS code
$\mathcal{Q}(A_j,A_{j+1}^T)$ are thus
$[[n_j,k_j,\min(d_j,\tilde{d}_{j})]]$, where 
 the distances in the corresponding co-chain
complex generated by transposed matrices $A_j^T$, 
$$
...
In the special cases $j=0$ (which formally corresponds to a zero-row
matrix $A_0$) it will be convenient to use $d_0=1$, and with $j=m-1$
(which formally corresponds to a zero-column matrix
$A_m=0_{n_{m-1}\times 0}$), use the distance $d_{m-1}$ of a binary code
$\mathcal{C}_{A_{m-1}}^\perp$ with the parity check matrix $A_{m-1}$
with the parameters $[n_{m-1},n_{m-1}-\rank A_{m-1},d_m]$.
...
$\tilde{\delta}\ge1$.  We use the convention that $\delta=\infty $ if
...
In many quantum error models, the probability of measurement error is related to the number of gates or qubits involved in each measurement. The row and column weight of the matrices in the complex chain can be used to determine upper and lower bound of the weight of check operators, and so the measurement error probability. It can easily be derived from the paramenter of $P$.

Let $\omega_j$ be the maximum weight of rows in $P_j$, and $\tilde\omega_j$ be the maximum weight of columns in $P_j$. The maximun row weight of $A^{(m)}_j$ would be  $\omega^{(m)}_j=\max( \omega^{(m-1)}_j+\omega_m,\omega^{(m-1)}_{j-1})$, and the maximun column weight $\tilde\omega^{(m)}_j=\max( \tilde\omega^{(m-1)}_{j-1}+\tilde\omega_m,\tilde\omega^{(m-1)}_{j})$. Apply the iteration rule, one can get the general expresion: $\displaystyle \omega^{(m)}_j=\max_{\{I^{(m-j+1)}\}} S^{(m)}(\{I^{(m-j+1)}\})$, 
...
In order to find the min weight $\omega'$, replace $\max$ with $\min$ and the same rules will apply. The difference is, when finding max weight, combination of rows or columns are not allowed. But when finding min weight, one can combine row or columns to get min weight.
...
Apply it to code $ {\cal Q}^{(m)}(A_{j},A_{j+1}^T)$, I will get \framebox{$[n^{(m)}_{j-1},\rank(A_j),\tilde\omega'^{(m)}_j]$} for the left part, here prime mean minimum instead of maximum. For the right part, the syndrome code has parameters  \framebox{$[n^{(m)}_{j+1},\rank(A_{j+1}),\omega'^{(m)}_j]$}. Combine them, I will get  \framebox{$[n^{(m)}_{j-1}+n^{(m)}_{j+1}, n^{(m)}_j-k^{(m)}_j ,\min(\tilde \omega'^{(m)}_j,\omega'^{(m)}_j)]$}
...
${\cal Q}_j\equiv {\cal Q}(A_{j},A_{j+1}^T)$, which I assume to encode
$k_j$ qubits in $n_j$.  Alternatively, $k_j$ is the rank of the
$j$\,th homology group in $\mathcal{K}$. 
...
$$
 ...
 d(Q,P)=\min_{\mathbf{x}\in {\cal C}_{Q}^\perp\setminus {\cal
     C}_{P}} \wgt \mathbf{x},
$$
so that the actual CSS code distance is the minimum of the two.  I
...
can be united if we take $d_0=1$.  In the following, we assume this to
be the case. 
\begin{theorem}
...
the more generous lower bound $d_j'\ge d_{j-1}\delta$.
\end{proof}
...
the Table \ref{tab:params} on page \pageref{tab:params} (assuming all
matrices have non-zero rank).
